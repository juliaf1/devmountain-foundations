...
let nameGreeting = nameCheck('Jojo');
...
const scream = string => console.log(string.toUpperCase());
scream(sampleString);
...
console.log(emailCheck('  julia@gmail.com  '));
...
const chocolateFrogPrice = 3;

const getChocolateFrogs = (gold) => {
  return Math.floor((eval(gold) / chocolateFrogPrice));
};

const getChange = (totalFrogs, gold) => {
  return eval(gold) % (chocolateFrogPrice * totalFrogs);
};

const callTrolleyWitch = (gold) => {
  let totalFrogs = getChocolateFrogs(gold);
  let change = getChange(totalFrogs, gold);
  let message = `Anything off the cart, dears?`;
  if (gold >= 3) {
    message += ` Sweet, you get ${totalFrogs} frog${pluralize(totalFrogs)}`;
    if (change > 0) {
      message += ` and ${change} galleon${pluralize(totalFrogs)} as change`;
    };
  } else {
    message += ` Sorry, darling, the frog is 3 galleons`;
  };
  return message;
};
...
console.log(callTrolleyWitch(9));
...
// Solution implemented in previous exercise
