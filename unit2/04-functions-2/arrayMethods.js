...
// CODE HERE
const totalPopulation = populations.reduce((previousNumber, currentNumber) => previousNumber + currentNumber);
...
const ordersTotals = orders.map(order => order.price += order.price * order.tax / 100);
...
const bobsTotal = purchases.filter(purchase => purchase.owner === 'Bob').reduce((total, currentOrder) => total + currentOrder.price, 0);
